...
function showNextCard(frontSideSelector) {
    ...
    currentCardIndex = (currentCardIndex + 1) % Object.keys(currentCardStorage).length;
    const obj_keys = Object.keys(currentCardStorage);
    const randomNextCard = obj_keys[currentCardIndex];

    if (!randomNextCard) {
        console.error("Die Karte konnte nicht abgerufen werden.");
        return;
    }

    console.log("Zeige Karte:", randomNextCard); // Debugging-Ausgabe
    ...
    // Vorderseite mit der Frage aktualisieren
...
document.getElementById("htmlButton").addEventListener("click", () => {
    const storedData = localStorage.getItem('cardStorageHTML');
...
document.getElementById("cssButton").addEventListener("click", () => {
    const storedData = localStorage.getItem('cardStorageCSS');
...
document.getElementById("jsButton").addEventListener("click", () => {
    const storedData = localStorage.getItem('cardStorageJS');
...
document.getElementById("nextCardButton").addEventListener("click", () => {
    ...
    if (aktStorageKey && currentCardStorage && Object.keys(currentCardStorage).length > 0) {
