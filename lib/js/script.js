...
function storageKeyButtonsLoad(){
    for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i); // Hole den Schlüssel
         // Hole alle relevanten Schlüssel
        const keys = Object.keys(localStorage).filter(key => key.startsWith("cardStorage"));
        if (keys.length === 0){
            document.querySelector('.cardsInStorage').style.display= 'none';
        } else {
            document.querySelector('.noCardsInStorage').style.display = 'none';
        }
...
function clickHinzufuegen(key, keySuffix) {
    document.getElementById(key).addEventListener('click', () => {
        ...
        storedData0 = localStorage.getItem(key); // Hole die gespeicherten Daten
...
function moveCardToNextLevelOrPrevious(currentKey, cardId, correct) {
    ...
    console.log('vorher ' +currentStorage.length)
    ...
    console.log('nachher '+ currentStorage.length)
