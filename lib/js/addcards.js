...
function loadKeysFromStorage() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key.startsWith("cardStorage")) {
      // Prüfe, ob der Schlüssel mit "cardStorage" beginnt
      console.log(key); // Gib den Schlüssel aus

      const keySuffix = key.replace("cardStorage", ""); // Entferne den Präfix

      // Prüfe, ob ein Button mit der ID bereits existiert
      if (!document.getElementById(key)) {
        // Füge den Button ein, wenn er nicht existiert
        savedCards.insertAdjacentHTML(
          "beforeend",
          `<button id='${key}'>${keySuffix}</button>`
        );
      }
      if (!document.getElementById(`delete${key}`)) {
        deleteKeyCards.insertAdjacentHTML(
          "beforeend",
          `<button id='delete${key}'>${keySuffix} löschen</button>`
        );
      }
